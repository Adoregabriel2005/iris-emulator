    ...
    : QDialog(parent), m_input(input)
{
    ...
    addCategory(tr("Jaguar Controls"), QStringLiteral("controller-line"));
    addCategory(tr("Jaguar"),         QStringLiteral("settings-3-line"));
    addCategory(tr("VST Plugins"),    QStringLiteral("volume-up-line"));
...
void SettingsDialog::createPages()
{
    ...
    m_page_stack->addWidget(createJaguarControllerPage());
    m_page_stack->addWidget(createJaguarPage());
    m_page_stack->addWidget(createVSTPage());
...
QWidget* SettingsDialog::createJaguarPage()
{
    ...
    // ── Hardware ──
    addSectionTitle(layout, tr("Hardware"));

    auto* gpuCheck = new QCheckBox(tr("Enable GPU (Tom)"), inner);
    gpuCheck->setObjectName("jag_gpu");
    gpuCheck->setChecked(settings.value("Jaguar/GPUEnabled", true).toBool());
    installTooltip(gpuCheck, tr("Enable the Jaguar GPU (Tom chip). Required by almost all games."));
    layout->addWidget(gpuCheck);

    auto* dspCheck = new QCheckBox(tr("Enable DSP (Jerry)"), inner);
    dspCheck->setObjectName("jag_dsp");
    dspCheck->setChecked(settings.value("Jaguar/DSPEnabled", true).toBool());
    installTooltip(dspCheck, tr("Enable the Jaguar DSP (Jerry chip). Required for audio. Disable only for debugging."));
    layout->addWidget(dspCheck);

    auto* fastBlitCheck = new QCheckBox(tr("Use fast blitter"), inner);
    fastBlitCheck->setObjectName("jag_fastblit");
    fastBlitCheck->setChecked(settings.value("Jaguar/FastBlitter", false).toBool());
    installTooltip(fastBlitCheck, tr("Use the faster but less accurate blitter. Some games will glitch with this enabled."));
    layout->addWidget(fastBlitCheck);

    // ── Video ──
    addSectionTitle(layout, tr("Video"));

    auto* tvRow = new QHBoxLayout();
    tvRow->addWidget(new QLabel(tr("TV Standard:"), inner));
    auto* jagTVCombo = new QComboBox(inner);
    jagTVCombo->setObjectName("jag_tv");
    jagTVCombo->addItem(tr("NTSC (60 Hz)"), QStringLiteral("NTSC"));
    jagTVCombo->addItem(tr("PAL (50 Hz)"),  QStringLiteral("PAL"));
    jagTVCombo->setCurrentIndex(jagTVCombo->findData(settings.value("Jaguar/TVStandard", "NTSC").toString()));
    installTooltip(jagTVCombo, tr("NTSC runs at 60 Hz (North America/Japan). PAL runs at 50 Hz (Europe) with extra scanlines."));
    tvRow->addWidget(jagTVCombo, 1);
    layout->addLayout(tvRow);

    // ── Jaguar Video Output Filter ──
    addSectionTitle(layout, tr("Video Output Filter"));

    auto* jagFilterHint = new QLabel(tr(
        "<i style='color:#aaa;'>The Jaguar supported RF, Composite, S-Video and RGB outputs.<br>"
        "Choose the cable type you want to simulate, or None for a clean digital image.</i>"), inner);
    jagFilterHint->setTextFormat(Qt::RichText);
    jagFilterHint->setWordWrap(true);
    layout->addWidget(jagFilterHint);

    auto* jagFilterRow = new QHBoxLayout();
    jagFilterRow->addWidget(new QLabel(tr("Output Filter:"), inner));
    auto* jagFilterCombo = new QComboBox(inner);
    jagFilterCombo->setObjectName("jag_filter");
    jagFilterCombo->addItem(tr("None (Clean Digital)"),                    QStringLiteral("none"));
    jagFilterCombo->addItem(tr("RF Cable — Noisy, chroma bleed, dot crawl"), QStringLiteral("rf"));
    jagFilterCombo->addItem(tr("S-Video — Sharp luma, soft chroma, CRT"),    QStringLiteral("svideo"));
    jagFilterCombo->addItem(tr("CRT Scanlines (generic)"),                  QStringLiteral("scanlines"));
    jagFilterCombo->addItem(tr("Smooth (Bilinear)"),                        QStringLiteral("smooth"));
    {
        QString cur = settings.value("Jaguar/Filter", "none").toString();
        int idx = jagFilterCombo->findData(cur);
        jagFilterCombo->setCurrentIndex(idx >= 0 ? idx : 0);
    }
    installTooltip(jagFilterCombo, tr(
        "RF: simulates the noisy RF cable signal with chroma bleed, dot crawl and luminance noise.\n"
        "S-Video: the best signal the Jaguar had — sharp luma, slightly soft chroma, mild CRT scanlines."));
    jagFilterRow->addWidget(jagFilterCombo, 1);
    layout->addLayout(jagFilterRow);

    auto* jagIntRow = new QHBoxLayout();
    jagIntRow->addWidget(new QLabel(tr("Filter Intensity:"), inner));
    auto* jagIntSpin = new QSpinBox(inner);
    jagIntSpin->setObjectName("jag_filter_intensity");
    jagIntSpin->setRange(0, 100);
    jagIntSpin->setSuffix("%");
    jagIntSpin->setValue(settings.value("Jaguar/FilterIntensity", 70).toInt());
    installTooltip(jagIntSpin, tr("How strong the selected filter effect is. 70% is a good default."));
    jagIntRow->addWidget(jagIntSpin, 1);
    layout->addLayout(jagIntRow);

    // ── Upscaling ──
    addSectionTitle(layout, tr("Upscaling"));

    auto* upscaleHint = new QLabel(tr(
        "<i style='color:#aaa;'>Upscaling renders the Jaguar image at a higher internal resolution.<br>"
        "Combined with S-Video or None filter for the cleanest result.</i>"), inner);
    upscaleHint->setTextFormat(Qt::RichText);
    upscaleHint->setWordWrap(true);
    layout->addWidget(upscaleHint);

    auto* upscaleRow = new QHBoxLayout();
    upscaleRow->addWidget(new QLabel(tr("Internal Resolution:"), inner));
    auto* upscaleCombo = new QComboBox(inner);
    upscaleCombo->setObjectName("jag_upscale");
    upscaleCombo->addItem(tr("Native (original resolution)"), QStringLiteral("native"));
    upscaleCombo->addItem(tr("480p"),                         QStringLiteral("480p"));
    upscaleCombo->addItem(tr("720p"),                         QStringLiteral("720p"));
    upscaleCombo->addItem(tr("1080p"),                        QStringLiteral("1080p"));
    {
        QString cur = settings.value("Jaguar/Upscale", "native").toString();
        int idx = upscaleCombo->findData(cur);
        upscaleCombo->setCurrentIndex(idx >= 0 ? idx : 0);
    }
    installTooltip(upscaleCombo, tr("Upscale the Jaguar framebuffer before applying filters. Higher = sharper but more GPU load."));
    upscaleRow->addWidget(upscaleCombo, 1);
    layout->addLayout(upscaleRow);

    // ── Aspect Ratio ──
    addSectionTitle(layout, tr("Aspect Ratio"));

    auto* arHint = new QLabel(tr(
        "<i style='color:#aaa;'>The Jaguar outputs at various resolutions (320×240 to 720×576).<br>"
        "4:3 matches a standard CRT TV. 16:9 fills a widescreen monitor without stretching.</i>"), inner);
    arHint->setTextFormat(Qt::RichText);
    arHint->setWordWrap(true);
    layout->addWidget(arHint);

    auto* arRow = new QHBoxLayout();
    arRow->addWidget(new QLabel(tr("Aspect Ratio:"), inner));
    auto* arCombo = new QComboBox(inner);
    arCombo->setObjectName("jag_aspect");
    arCombo->addItem(tr("Auto (source pixel aspect)"), QStringLiteral("auto"));
    arCombo->addItem(tr("4:3 (standard CRT TV)"),      QStringLiteral("4:3"));
    arCombo->addItem(tr("16:9 (widescreen)"),           QStringLiteral("16:9"));
    {
        QString cur = settings.value("Jaguar/AspectRatio", "4:3").toString();
        int idx = arCombo->findData(cur);
        arCombo->setCurrentIndex(idx >= 0 ? idx : 1);
    }
    installTooltip(arCombo, tr(
        "Auto: uses the raw pixel dimensions.\n"
        "4:3: correct for a standard TV (recommended for most games).\n"
        "16:9: fills a widescreen display with pillarbox-free image."));
    arRow->addWidget(arCombo, 1);
    layout->addLayout(arRow);

    // ── BIOS / Intro ──
    addSectionTitle(layout, tr("BIOS / Boot Intro"));

    auto* biosCheck = new QCheckBox(tr("Use Jaguar BIOS (shows boot intro)"), inner);
    biosCheck->setObjectName("jag_bios");
    biosCheck->setChecked(settings.value("Jaguar/UseBIOS", false).toBool());
    installTooltip(biosCheck, tr(
        "Load the Jaguar boot ROM before the cartridge. "
        "Shows the official Atari Jaguar intro animation. "
        "Requires jaguar.j64 or jaguar.rom in the emulator folder."));
    layout->addWidget(biosCheck);

    auto* biosRow = new QHBoxLayout();
    biosRow->addWidget(new QLabel(tr("BIOS file:"), inner));
    auto* biosEdit = new QLineEdit(inner);
    biosEdit->setObjectName("jag_bios_path");
    biosEdit->setText(settings.value("Jaguar/BIOSPath", "").toString());
    biosEdit->setPlaceholderText(tr("jaguar.j64 / jaguar.rom (optional)"));
    installTooltip(biosEdit, tr("Path to the Jaguar BIOS ROM file. Leave blank to auto-detect next to the executable."));
    biosRow->addWidget(biosEdit, 1);
    auto* biosBrowse = new QPushButton(tr("Browse..."), inner);
    connect(biosBrowse, &QPushButton::clicked, this, [biosEdit, this]() {
        QString p = QFileDialog::getOpenFileName(this, tr("Select Jaguar BIOS"), {},
            tr("Jaguar BIOS (*.j64 *.rom *.bin);;All Files (*)"));
        if (!p.isEmpty()) biosEdit->setText(p);
    });
    biosRow->addWidget(biosBrowse);
    layout->addLayout(biosRow);

    auto* biosHint = new QLabel(tr(
        "<i style='color:#888;'>The BIOS file is copyrighted by Atari and cannot be provided.<br>"
        "You must dump it from your own Jaguar hardware.</i>"), inner);
    biosHint->setWordWrap(true);
    layout->addWidget(biosHint);

    // ── EEPROMs ──
    addSectionTitle(layout, tr("EEPROMs (Save Data)"));

    auto* eepromRow = new QHBoxLayout();
    eepromRow->addWidget(new QLabel(tr("EEPROMs folder:"), inner));
    auto* eepromEdit = new QLineEdit(inner);
    eepromEdit->setObjectName("jag_eeprom_path");
    eepromEdit->setText(settings.value("Jaguar/EEPROMPath", "eeproms").toString());
    installTooltip(eepromEdit, tr("Folder where Jaguar EEPROM save files are stored (high scores, game saves)."));
    eepromRow->addWidget(eepromEdit, 1);
    auto* eepromBrowse = new QPushButton(tr("Browse..."), inner);
    connect(eepromBrowse, &QPushButton::clicked, this, [eepromEdit, this]() {
        QString d = QFileDialog::getExistingDirectory(this, tr("Select EEPROMs Folder"));
        if (!d.isEmpty()) eepromEdit->setText(d);
    });
    eepromRow->addWidget(eepromBrowse);
    layout->addLayout(eepromRow);
...
QWidget* SettingsDialog::createVSTPage()
{
    auto* page   = new QWidget(this);
    auto* layout = new QVBoxLayout(page);
    layout->setContentsMargins(8, 4, 8, 4);

    addSectionTitle(layout, tr("VST3 Plugin Chain"));

    // Enable/disable chain toggle
    auto* chainEnableCheck = new QCheckBox(tr("Enable VST chain"), page);
    chainEnableCheck->setChecked(VSTHost::instance().isChainEnabled());
    installTooltip(chainEnableCheck, tr("Enable or bypass the entire VST3 plugin chain. When disabled, audio passes through unprocessed."));
    layout->addWidget(chainEnableCheck);
    connect(chainEnableCheck, &QCheckBox::toggled, this, [](bool on) {
        VSTHost::instance().setChainEnabled(on);
    });

    auto* hint = new QLabel(tr(
        "<i>VST3 plugins are applied to the audio output in real-time.<br>"
        "Drag to reorder. Double-click a plugin to open its editor.</i>"), page);
    hint->setWordWrap(true);
    hint->setStyleSheet("color:#aaa;");
    layout->addWidget(hint);

    // Plugin list
    auto* pluginList = new QListWidget(page);
    pluginList->setObjectName("vst_list");
    pluginList->setDragDropMode(QAbstractItemView::InternalMove);
    pluginList->setSelectionMode(QAbstractItemView::SingleSelection);
    pluginList->setMinimumHeight(160);
    installTooltip(pluginList, tr("Active VST3 plugin chain. Drag to reorder. Double-click to open plugin editor."));

    // Populate from VSTHost
    auto refreshList = [pluginList]() {
        pluginList->clear();
        VSTHost& h = VSTHost::instance();
        for (int i = 0; i < h.pluginCount(); i++) {
            const auto& info = h.pluginInfo(i);
            auto* item = new QListWidgetItem(
                QIcon::fromTheme("volume-up-line"),
                info.name + (info.enabled ? "" : tr(" [disabled]")));
    ...
    connect(pluginList, &QListWidget::itemChanged, this, [pluginList](QListWidgetItem* item) {
        int idx = item->data(Qt::UserRole).toInt();
        VSTHost::instance().setPluginEnabled(idx, item->checkState() == Qt::Checked);
    });

    // Double-click opens the plugin's native editor
    connect(pluginList, &QListWidget::itemDoubleClicked, this, [this](QListWidgetItem* item) {
        int idx = item->data(Qt::UserRole).toInt();
        if (!VSTHost::instance().openEditor(idx, reinterpret_cast<void*>(winId())))
            QMessageBox::information(this, tr("VST Editor"),
                tr("This plugin does not have a graphical editor."));
    });
    ...
    auto* addBtn = new QPushButton(tr("Add Plugin..."), page);
    installTooltip(addBtn, tr("Load a VST3 plugin (.vst3) and add it to the chain."));
    connect(addBtn, &QPushButton::clicked, this, [pluginList, refreshList, this]() {
        QString path = QFileDialog::getOpenFileName(this, tr("Select VST3 Plugin"), {},
            tr("VST3 Plugins (*.vst3);;All Files (*)"));
        if (path.isEmpty()) return;
        if (!VSTHost::instance().addPlugin(path))
            QMessageBox::warning(this, tr("VST Error"), tr("Failed to load plugin:\n%1").arg(path));
        else
            refreshList();
    });
    btnRow->addWidget(addBtn);

    auto* scanBtn = new QPushButton(tr("Scan Folder..."), page);
    installTooltip(scanBtn, tr("Scan a folder for VST3 plugins and add all found."));
    connect(scanBtn, &QPushButton::clicked, this, [pluginList, refreshList, this]() {
        QString dir = QFileDialog::getExistingDirectory(this, tr("Select VST3 Folder"));
        if (dir.isEmpty()) return;
        QStringList found = VSTHost::instance().scanDirectory(dir);
        int added = 0;
        for (const QString& p : found)
            if (VSTHost::instance().addPlugin(p)) added++;
        refreshList();
        QMessageBox::information(this, tr("Scan Complete"),
            tr("Found %1 plugin(s), loaded %2.")
            .arg(found.size()).arg(added));
    });
    btnRow->addWidget(scanBtn);

    auto* removeBtn = new QPushButton(tr("Remove"), page);
    installTooltip(removeBtn, tr("Remove the selected plugin from the chain."));
    connect(removeBtn, &QPushButton::clicked, this, [pluginList, refreshList]() {
        int row = pluginList->currentRow();
        if (row < 0) return;
        VSTHost::instance().removePlugin(row);
        refreshList();
    });
    ...
    // Master gain
    addSectionTitle(layout, tr("Master Gain"));
    auto* gainRow = new QHBoxLayout();
    gainRow->addWidget(new QLabel(tr("Output Gain:"), page));
    auto* gainSlider = new QSlider(Qt::Horizontal, page);
    gainSlider->setRange(0, 200);  // 0% – 200%
    gainSlider->setValue(static_cast<int>(VSTHost::instance().masterGain() * 100.f));
    auto* gainLabel = new QLabel(
        QString::number(gainSlider->value()) + "%", page);
    gainLabel->setFixedWidth(40);
    connect(gainSlider, &QSlider::valueChanged, this, [gainLabel](int v) {
        gainLabel->setText(QString::number(v) + "%");
        VSTHost::instance().setMasterGain(v / 100.f);
    });
    installTooltip(gainSlider, tr("Master output gain after the VST chain. 100% = unity gain."));
    gainRow->addWidget(gainSlider, 1);
    gainRow->addWidget(gainLabel);
    layout->addLayout(gainRow);

    // Default VST3 paths hint
    auto* pathHint = new QLabel(tr(
        "<i style='color:#888;'>Default VST3 locations on Windows:<br>"
        "C:\\Program Files\\Common Files\\VST3<br>"
        "C:\\Program Files (x86)\\Common Files\\VST3</i>"), page);
...
// Draws the real Atari Jaguar controller: D-pad, 3 face buttons (A/B/C),
// Option, Pause, and the 12-key numpad (0-9, *, #).
// Click any zone to rebind it.
...
void JaguarJoystickWidget::buildZones()
{
    ...
    // ── D-pad (top-left) ──────────────────────────────────────────────────────
    m_zones += {
        {{0.08, 0.04, 0.12, 0.09}, (int)InputAction::JagUp,     "UP"},
        {{0.08, 0.22, 0.12, 0.09}, (int)InputAction::JagDown,   "DOWN"},
        {{0.01, 0.13, 0.10, 0.09}, (int)InputAction::JagLeft,   "LEFT"},
        {{0.15, 0.13, 0.10, 0.09}, (int)InputAction::JagRight,  "RIGHT"},
    };
    // ── Face buttons A/B/C (top-right) ───────────────────────────────────────
    m_zones += {
        {{0.62, 0.04, 0.11, 0.09}, (int)InputAction::JagC,      "C"},
        {{0.74, 0.04, 0.11, 0.09}, (int)InputAction::JagB,      "B"},
        {{0.86, 0.04, 0.11, 0.09}, (int)InputAction::JagA,      "A"},
    };
    // ── Option / Pause (center top) ──────────────────────────────────────────
    m_zones += {
        {{0.34, 0.04, 0.13, 0.07}, (int)InputAction::JagOption, "OPTION"},
        {{0.50, 0.04, 0.13, 0.07}, (int)InputAction::JagPause,  "PAUSE"},
    };
    // ── Numpad 3x4 (bottom half) ─────────────────────────────────────────────
    // Row 1: 1 2 3
    m_zones += {
        {{0.18, 0.38, 0.17, 0.10}, (int)InputAction::Jag1, "1"},
        {{0.40, 0.38, 0.17, 0.10}, (int)InputAction::Jag2, "2"},
        {{0.62, 0.38, 0.17, 0.10}, (int)InputAction::Jag3, "3"},
    };
    // Row 2: 4 5 6
    m_zones += {
        {{0.18, 0.51, 0.17, 0.10}, (int)InputAction::Jag4, "4"},
        {{0.40, 0.51, 0.17, 0.10}, (int)InputAction::Jag5, "5"},
        {{0.62, 0.51, 0.17, 0.10}, (int)InputAction::Jag6, "6"},
    };
    // Row 3: 7 8 9
    m_zones += {
        {{0.18, 0.64, 0.17, 0.10}, (int)InputAction::Jag7, "7"},
        {{0.40, 0.64, 0.17, 0.10}, (int)InputAction::Jag8, "8"},
        {{0.62, 0.64, 0.17, 0.10}, (int)InputAction::Jag9, "9"},
    };
    // Row 4: * 0 #
    m_zones += {
        {{0.18, 0.77, 0.17, 0.10}, (int)InputAction::JagStar, "*"},
        {{0.40, 0.77, 0.17, 0.10}, (int)InputAction::Jag0,    "0"},
        {{0.62, 0.77, 0.17, 0.10}, (int)InputAction::JagHash, "#"},
    };
}
...
void JaguarJoystickWidget::paintEvent(QPaintEvent*)
{
    ...
    // ── Controller body ───────────────────────────────────────────────────────
    // Main body — dark grey, portrait orientation
    QRectF body(w*0.04, h*0.01, w*0.92, h*0.97);
    p.setPen(Qt::NoPen);
    p.setBrush(QColor(45, 45, 50));
    p.drawRoundedRect(body, 16, 16);

    // Top grip bumps (left and right)
    p.setBrush(QColor(38, 38, 43));
    p.drawRoundedRect(QRectF(w*0.04, h*0.01, w*0.20, h*0.12), 10, 10);
    p.drawRoundedRect(QRectF(w*0.76, h*0.01, w*0.20, h*0.12), 10, 10);

    // ── D-pad area (top-left) ─────────────────────────────────────────────────
    QPointF dc(w*0.14, h*0.17);
    double ds = qMin(w,h)*0.12;
    p.setBrush(QColor(30,30,35));
    p.drawEllipse(dc, ds*1.1, ds*1.1);
    double aw = ds*0.36, al = ds*0.92;
    p.setBrush(QColor(60,60,65));
    p.drawRoundedRect(QRectF(dc.x()-aw/2, dc.y()-al, aw, al*2), 3, 3);
    p.drawRoundedRect(QRectF(dc.x()-al,   dc.y()-aw/2, al*2, aw), 3, 3);
    p.setBrush(QColor(50,50,55));
    p.drawEllipse(dc, aw*0.5, aw*0.5);

    // ── Face buttons A/B/C (top-right) ───────────────────────────────────────
    // C=blue, B=yellow, A=red (Jaguar colors)
    struct BtnDef { double cx, cy; QColor col; const char* lbl; };
    BtnDef btns[] = {
        {0.68, 0.09, QColor(40,80,180),  "C"},
        {0.79, 0.09, QColor(180,160,20), "B"},
        {0.90, 0.09, QColor(180,40,40),  "A"},
    };
    double br = w*0.030;
    QFont bf = font(); bf.setPixelSize(10); bf.setBold(true); p.setFont(bf);
    for (auto& b : btns) {
        QPointF bc(w*b.cx, h*b.cy);
        p.setBrush(b.col.darker(140)); p.drawEllipse(bc, br*1.3, br*1.3);
        p.setBrush(b.col);             p.drawEllipse(bc, br,     br);
        p.setPen(Qt::white);
        p.drawText(QRectF(bc.x()-br, bc.y()-br, br*2, br*2), Qt::AlignCenter, b.lbl);
        p.setPen(Qt::NoPen);
    }

    // ── Option / Pause (center top) ───────────────────────────────────────────
    p.setBrush(QColor(55,55,60));
    p.drawRoundedRect(QRectF(w*0.34, h*0.04, w*0.13, h*0.07), 4, 4);
    p.drawRoundedRect(QRectF(w*0.50, h*0.04, w*0.13, h*0.07), 4, 4);
    QFont sf = font(); sf.setPixelSize(8); p.setFont(sf);
    p.setPen(QColor(160,160,170));
    p.drawText(QRectF(w*0.34, h*0.04, w*0.13, h*0.07), Qt::AlignCenter, "OPTION");
    p.drawText(QRectF(w*0.50, h*0.04, w*0.13, h*0.07), Qt::AlignCenter, "PAUSE");
    p.setPen(Qt::NoPen);

    // ── Numpad area (bottom half) ─────────────────────────────────────────────
    QRectF numArea(w*0.12, h*0.33, w*0.76, h*0.60);
    p.setBrush(QColor(35,35,40));
    p.drawRoundedRect(numArea, 10, 10);

    // Numpad label
    QFont nf = font(); nf.setPixelSize(9); p.setFont(nf);
    p.setPen(QColor(100,100,110));
    p.drawText(QRectF(w*0.12, h*0.33, w*0.76, h*0.06), Qt::AlignCenter, "NUMPAD");
    p.setPen(Qt::NoPen);

    // ── Hot zones ─────────────────────────────────────────────────────────────
    QFont lf = font(); lf.setPixelSize(10); lf.setBold(true); p.setFont(lf);
    for (const auto& zone : m_zones) {
        QRectF r(zone.rect.x()*w, zone.rect.y()*h, zone.rect.width()*w, zone.rect.height()*h);
        bool cap = (m_capture_action == zone.action);
        p.setBrush(cap ? QColor(0,120,255,210) : QColor(0,88,208,150));
        p.setPen(QPen(QColor(255,255,255,80), 1));
        p.drawRoundedRect(r, 5, 5);
        p.setPen(Qt::white);
        QString txt = zone.label;
        if (cap) {
            txt = tr("Press...");
        } else {
            InputBinding b = m_input->getBinding(static_cast<InputAction>(zone.action));
            if (b.isValid()) txt += "\n" + b.toDisplayString();
        }
...
void JaguarJoystickWidget::mousePressEvent(QMouseEvent* event)
{
    ...
    for (const auto& zone : m_zones) {
        QRectF r(zone.rect.x()*w, zone.rect.y()*h, zone.rect.width()*w, zone.rect.height()*h);
        if (r.contains(pos)) { startCapture(zone.action); return; }
    }
}

void JaguarJoystickWidget::startCapture(int action)
{
    m_capture_action = action;
    m_capture_countdown = 100;
    setFocusPolicy(Qt::StrongFocus); setFocus(); grabKeyboard();
    m_capture_timer->start(); update();
}

void JaguarJoystickWidget::stopCapture()
{
    m_capture_action = -1;
    m_capture_timer->stop(); releaseKeyboard();
    setFocusPolicy(Qt::NoFocus);
    if (m_input) m_input->clearAllKeyStates();
    update();
}

void JaguarJoystickWidget::keyPressEvent(QKeyEvent* event)
{
    if (m_capture_action >= 0) {
        if (m_input && !event->isAutoRepeat()) m_input->setQtKeyState(event->key(), true);
        event->accept(); return;
    }
    QWidget::keyPressEvent(event);
}

void JaguarJoystickWidget::keyReleaseEvent(QKeyEvent* event)
{
    if (m_capture_action >= 0) {
        if (m_input && !event->isAutoRepeat()) m_input->setQtKeyState(event->key(), false);
        event->accept(); return;
    }
    QWidget::keyReleaseEvent(event);
}

void JaguarJoystickWidget::focusOutEvent(QFocusEvent* event)
{
    if (m_capture_action >= 0) stopCapture();
    QWidget::focusOutEvent(event);
}
...
void JaguarJoystickWidget::onCaptureTimerTick()
{
    if (m_capture_action < 0) { stopCapture(); return; }
    if (--m_capture_countdown <= 0) { stopCapture(); return; }
    InputBinding b = m_input->pollForBinding(true, true);
    if (b.isValid()) {
        m_input->setBinding(static_cast<InputAction>(m_capture_action), b);
        m_input->saveBindings();
        stopCapture();
        emit bindingChanged();
    }
...
QWidget* SettingsDialog::createJaguarControllerPage()
{
    auto* page   = new QWidget(this);
    auto* layout = new QVBoxLayout(page);
    layout->setContentsMargins(8, 4, 8, 4);
    ...
    auto* info = new QLabel(tr(
        "Click any button on the controller below to rebind it.\n"
        "Works with keyboard, Xbox, PlayStation, Switch Pro, x360ce and generic USB gamepads."), page);
    info->setWordWrap(true);
    info->setStyleSheet("color:#aaa;");
    layout->addWidget(info);

    // Gamepad status
    auto* gpRow = new QHBoxLayout();
    auto* gpLabel = new QLabel(
        m_input->hasGamepad()
            ? tr("✔ Gamepad: %1").arg(m_input->gamepadName())
            : tr("No gamepad detected — keyboard only."), page);
    gpLabel->setStyleSheet("color:#aaa;");
    gpRow->addWidget(gpLabel, 1);
    layout->addLayout(gpRow);

    m_jaguar_joystick_widget = new JaguarJoystickWidget(m_input, page);
    layout->addWidget(m_jaguar_joystick_widget, 1);

    // Default mapping hint
    auto* hint = new QLabel(tr(
        "<i style='color:#888;'>"
        "Default gamepad mapping (Xbox/PS/Switch):<br>"
        "D-pad / Left stick = directions &nbsp;·&nbsp; "
        "B/Circle = Jag A &nbsp;·&nbsp; A/Cross = Jag B &nbsp;·&nbsp; X/Square = Jag C<br>"
        "LB/L1 = Option &nbsp;·&nbsp; Start = Pause &nbsp;·&nbsp; "
        "RT = Jag A (alt) &nbsp;·&nbsp; LT = Jag B (alt)<br>"
        "Numpad 0-9 / KP0-9 = Jag 0-9 &nbsp;·&nbsp; KP* = * &nbsp;·&nbsp; KP- = #"
        "</i>"), page);
    hint->setWordWrap(true);
    layout->addWidget(hint);
    ...
    auto* resetRow = new QHBoxLayout();
    resetRow->addStretch();
    auto* resetBtn = new QPushButton(tr("Reset Jaguar Defaults"), page);
    installTooltip(resetBtn, tr("Restore all Jaguar controller bindings to factory defaults."));
    connect(resetBtn, &QPushButton::clicked, this, [this]() {
        for (int i = (int)InputAction::JagUp; i < (int)InputAction::Count; i++) {
            auto a = static_cast<InputAction>(i);
            m_input->setBinding(a, SDLInput::defaultBinding(a));
        }
        m_input->saveBindings();
        m_jaguar_joystick_widget->update();
    });
    resetRow->addWidget(resetBtn);
    layout->addLayout(resetRow);
...
void SettingsDialog::onAccepted()
{
    ...
    // Jaguar
    if (auto* w = findChild<QCheckBox*>("jag_gpu"))
        settings.setValue("Jaguar/GPUEnabled", w->isChecked());
    if (auto* w = findChild<QCheckBox*>("jag_dsp"))
        settings.setValue("Jaguar/DSPEnabled", w->isChecked());
    if (auto* w = findChild<QCheckBox*>("jag_fastblit"))
        settings.setValue("Jaguar/FastBlitter", w->isChecked());
    if (auto* w = findChild<QComboBox*>("jag_tv"))
        settings.setValue("Jaguar/TVStandard", w->currentData().toString());
    if (auto* w = findChild<QCheckBox*>("jag_bios"))
        settings.setValue("Jaguar/UseBIOS", w->isChecked());
    if (auto* w = findChild<QLineEdit*>("jag_bios_path"))
        settings.setValue("Jaguar/BIOSPath", w->text());
    if (auto* w = findChild<QLineEdit*>("jag_eeprom_path"))
        settings.setValue("Jaguar/EEPROMPath", w->text());
    if (auto* w = findChild<QComboBox*>("jag_filter"))
        settings.setValue("Jaguar/Filter", w->currentData().toString());
    if (auto* w = findChild<QSpinBox*>("jag_filter_intensity"))
        settings.setValue("Jaguar/FilterIntensity", w->value());
    if (auto* w = findChild<QComboBox*>("jag_upscale"))
        settings.setValue("Jaguar/Upscale", w->currentData().toString());
    if (auto* w = findChild<QComboBox*>("jag_aspect"))
        settings.setValue("Jaguar/AspectRatio", w->currentData().toString());

    // VST chain
    VSTHost::instance().saveChain();
